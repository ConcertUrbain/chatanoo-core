<?php
	...
	class Table_Medias_SoundTest extends PHPUnit_Extensions_Database_TestCase
	{
		/**
		 * Table users
		 *
		...
		{
			$mediaRowArray = $this->_mediasSoundTable->createRow()->toArray();
		...
		{
			$mediaRow = $this->_mediasSoundTable->find(1)->current();
			$this->assertEquals($mediaRow->id, 1);
			$this->assertEquals($mediaRow->title, 'Un son');
			$this->assertEquals($mediaRow->description, 'il s ecoute');
			$this->assertEquals($mediaRow->url, 'http://www.unsite.fr/sound.mp3');
		...
		public function testAddSound()
		{
			$mediaArray = array(
				'title' => 'Un son 2',
				'description' => 'il s ecoute 2',
				'url' => 'http://www.unsite.fr/sound2.mp3',
				'totalTime' => 180,
			...
			$mediaRow = $this->_mediasSoundTable->createRow($mediaArray);
			$mediaRow->save();

			$insertXml = dirname(__FILE__) . '/../Flats/mediassound-insert-data.xml';
			$this->assertDataSetsEqual(
		...
		public function testDeleteComment()
		{
			$mediaRow = $this->_mediasSoundTable->find(1)->current();
			$mediaRow->delete();
			$this->assertEquals($this->_mediasSoundTable->fetchAll()->count(), 0);
		}
		...
		public function testGetParentDatasAdress()
		{
			...
			$select->from('items', 'datas_adress.*')
					...
					->where("datas_assoc.assocType = 'Media_Sound'");
