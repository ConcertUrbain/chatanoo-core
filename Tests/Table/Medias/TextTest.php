<?php
	...
	class Table_Medias_TextTest extends PHPUnit_Extensions_Database_TestCase
	{
		/**
		 * Table users
		 *
		...
		{
			$mediaRowArray = $this->_mediasTextTable->createRow()->toArray();
			...
			$this->assertArrayHasKey('content', $mediaRowArray);
		...
		{
			$mediaRow = $this->_mediasTextTable->find(1)->current();
			$this->assertEquals($mediaRow->id, 1);
			$this->assertEquals($mediaRow->title, 'Un texte');
			$this->assertEquals($mediaRow->description, 'il se lit');
			$this->assertEquals($mediaRow->content, 'Un texte');
		...
		public function testAddText()
		{
			...
			$mediaRow = $this->_mediasTextTable->createRow($mediaArray);
			$mediaRow->save();

			$insertXml = dirname(__FILE__) . '/../Flats/mediastext-insert-data.xml';
			$this->assertDataSetsEqual(
		...
		public function testDeleteComment()
		{
			$mediaRow = $this->_mediasTextTable->find(1)->current();
			$mediaRow->delete();
			$this->assertEquals($this->_mediasTextTable->fetchAll()->count(), 0);
		}
		...
		public function testGetParentDatasAdress()
		{
			...
			$select->from('items', 'datas_adress.*')
					...
					->where("datas_assoc.assocType = 'Media_Text'");
